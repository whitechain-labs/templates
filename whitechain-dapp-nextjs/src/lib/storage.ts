 ...
 * `retrieve() -> uint256` contract shipped by the companion Whitechain Hardhat and
 * Foundry templates. Declared `as const` so wagmi can fully type `functionName`,
 * `args` and return values at the call sites.
 */
 ...
 * `NEXT_PUBLIC_STORAGE_ADDRESS` to point at your own deployment.
...
const rawAddress = process.env.NEXT_PUBLIC_STORAGE_ADDRESS?.trim() || DEFAULT_STORAGE_ADDRESS;
