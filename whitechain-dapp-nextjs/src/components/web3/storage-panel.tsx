...
/**
 * Client island for the example Storage contract. `retrieve()` is a public read,
 * so it runs over the RPC as soon as an address is configured (no wallet needed);
 * `store(uint256)` is a write via wagmi's `useWriteContract` and is gated on a
 * connected wallet on the right network.
 */
  ...
  if (!storageAddress) {
    return (
      <p className="text-sm text-gray-500">
        NEXT_PUBLIC_STORAGE_ADDRESS is not a valid contract address. Set it to a deployed Storage
        address (0x…) or leave it blank to use the default.
      </p>
    );
  }
